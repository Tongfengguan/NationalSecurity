...
const domains = [
  ...
  { id: 'cyber', name: '网络安全', en: 'CYBER_SHIELD', desc: 'NET_SOVEREIGNTY / 构筑清朗安全的数字空间防线。', img: 'https://images.weserv.nl/?url=https://images.unsplash.com/photo-1550751827-4bd374c3f58b?w=1200' },
...
<template>
  <div class="command-container">
    ...
    <section class="ppt-section hero" data-index="0">
      ...
      <div class="content">
        <div class="reveal-snappy mono alert-tag">>> AUTHENTICATED_ACCESS_ONLY</div>
        <h1 class="hero-title serif reveal-snappy">总体国家安全观</h1>
        <div class="terminal-output mono reveal-snappy">
          [SYS] INITIALIZING_SECURITY_MODULE... OK<br/>
          [SYS] LOADING_DATABASE_CORE... OK<br/>
          [SYS] ENCRYPTING_DATA_STREAM... OK
        </div>
        <div class="scroll-arrow mono reveal-snappy">SCROLL_TO_INIT_SCAN v</div>
      </div>
    </section>
    ...
    >
      ...
      <div class="content-box">
        <div class="status-header mono reveal-snappy">
          <span class="pulse"></span> LIVE_MONITORING: {{ item.id.toUpperCase() }}
        </div>
    ...
    <section class="ppt-section footer-page" :data-index="domains.length + 1">
      <div class="content text-center">
        ...
        <div class="mono alert-banner">!! THREAT_LEVEL_MINIMAL !!</div>
...
.command-container {
  ...
  scroll-behavior: steps(10); /* 增加滑动时的机械感 */
  background: #000;
  margin: -60px 0 0 0;
}
...
.hud-nav-item { font-size: 0.6rem; color: #222; transition: all 0.2s; }
...
.hud-corner {
  ...
  border-color: var(--alert-red); border-style: solid;
  z-index: 11;
}
...
.status-header { font-size: 0.7rem; color: var(--alert-red); margin-bottom: 20px; display: flex; align-items: center; gap: 8px; letter-spacing: 2px; }
.pulse { width: 8px; height: 8px; background: var(--alert-red); animation: pulse 1s infinite; }
@keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.3; } 100% { opacity: 1; } }

.hero-title { font-size: clamp(3rem, 10vw, 6rem); font-weight: 900; margin: 20px 0; }
.alert-tag { color: var(--alert-red); font-size: 0.8rem; letter-spacing: 2px; }
.terminal-output { font-size: 0.8rem; color: #444; line-height: 1.5; border-left: 2px solid #222; padding-left: 20px; margin-top: 30px; }

.en-code { font-size: 0.8rem; color: var(--alert-red); margin-bottom: 10px; letter-spacing: 4px; }
.title { font-size: clamp(2.5rem, 8vw, 5rem); color: #fff; margin: 0; }
.divider-hard { width: 100px; height: 4px; background: #fff; margin: 30px 0; }
.description { font-size: 1rem; color: #888; max-width: 450px; line-height: 1.8; margin-bottom: 40px; }

.access-btn {
  background: transparent; border: 1px solid #fff; color: #fff;
  padding: 12px 25px; font-size: 0.8rem; cursor: pointer;
  transition: all 0.2s;
}
.access-btn:hover { background: #fff; color: #000; }
.access-btn.large { padding: 15px 40px; font-size: 1rem; border-color: var(--alert-red); color: var(--alert-red); }
.access-btn.large:hover { background: var(--alert-red); color: #fff; }

.watermark {
  position: absolute; right: 50px; bottom: 40px;
  font-size: 15vw; font-weight: 900; color: #111;
  pointer-events: none;
}

.hotline-hud { margin: 40px 0; }
.hotline-hud .l { font-size: 0.8rem; color: #444; display: block; margin-bottom: 10px; }
.hotline-hud .n { font-size: 6rem; color: var(--alert-red); }

@media (max-width: 768px) {
  .hud-frame, .hud-corner, .hud-side-nav { display: none; }
  .content-box { padding: 0 30px; }
  .hotline-hud .n { font-size: 4rem; }
  .watermark { font-size: 30vw; }
}
</style>